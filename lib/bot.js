...
class Bot extends EventEmitter {
    ...
    bindEventHandlers() {
        ...
        this.csgoClient.on('inspectItemInfo', (itemData) => {
            if (this.resolve && this.currentRequest) {
                ...
                // Ensure the received itemid is the same as what we want when known.
                // Newer embedded inspect payloads don't include an asset ID until response time.
                if (!this.currentRequest.link.hasEmbeddedData && itemData.iteminfo.itemid !== this.currentRequest.a) {
                    return;
                }

                if (this.currentRequest.link.hasEmbeddedData && itemData.iteminfo.itemid) {
                    this.currentRequest.link.setResolvedAssetId(itemData.iteminfo.itemid);
                }
                ...
                const resolvedAssetId = this.currentRequest.link.getParams().a;

                itemData.iteminfo.s = this.currentRequest.s;
                itemData.iteminfo.a = resolvedAssetId;
    ...
    sendFloatRequest(link) {
        return new Promise((resolve, reject) => {
            ...
            this.currentRequest = {s: params.s, a: params.a, d: params.d, m: params.m, link, time: new Date().getTime()};

            if (!this.ready) {
                reject('This bot is not ready');
            }
            else {
                // Let node-globaloffensive parse the entire inspect link.
                // This supports both legacy links and newer embedded inspect payloads.
                this.csgoClient.inspectItem(link.getLink());
            }
